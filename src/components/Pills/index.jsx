...
function Pills({ types }) {
  ...
  return (
    ...
    { types.map((item, index) => (
        <Link to={`/${item}`} key={index} className={(filter === `${item}`) ? "inline-block rounded-xl py-1 px-3 bg-amber-400 text-slate-700 font-semibold " : "inline-block rounded-xl py-1 px-3 bg-slate-300 text-white font-semibold" }>{translateType(item)}</Link>
    )) }
</div>
  )
}
